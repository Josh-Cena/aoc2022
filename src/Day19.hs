...
data State = State
  { ore :: Int, clay :: Int, obsidian :: Int, geode :: Int,
    oreBots :: Int, clayBots :: Int, obsidianBots :: Int, geodeBots :: Int,
    time :: Int
  }
  deriving (Show)
...
buildBot ty bp st = case ty of
  ...
  Obsidian -> let (oreSpent, claySpent) = obsidianReq bp in st'
    { obsidianBots = obsidianBots st' + 1,
      ore = ore st' - oreSpent,
      clay = clay st' - claySpent
    }
  Geode -> let (oreSpent, obsidianSpent) = geodeReq bp in st'
