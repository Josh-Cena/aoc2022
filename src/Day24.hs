...
canMoveTo board width height (r, c, t) =
  case M.lookup (r, c) board of
    ...
    _ ->
      ...
      in all (\(pos, wind) -> M.lookup pos board /= Just wind) blockingWinds

bfs :: Map (Int, Int) Char
  -> Int
  -> Int
  -> (Int, Int, Int)
  -> (Int, Int)
...
bfs board width height start (er, ec) =
  go (Seq.singleton start) (S.singleton start)
  where
    ...
    go (s@(r, c, t) Seq.:<| q) seen
      ...
      | otherwise =
        let
          next = [ n | n <- neighbors s, canMoveTo board width height n, n `S.notMember` seen ]
